<template>
  <div class="login-box">
    ...
    <div class="login-block input-box">
      ...
      <el-input v-model="againPwd" placeholder="请输入确认密码"></el-input>
    </div>
    <div class="login-block btn" @click="toLog">
      <p>注册</p>
    </div>
    <div class="rule">
      注册即表示已经阅读并同意<span>《用户服务条款》</span>及<span>《隐私保护协议》</span>
    </div>
    <div class="toRegister">
      已有账号？去<span @click="toRegister">登录</span>
    </div>
  </div>
</template>
  ...
  methods: {
    toLog () {
      if (!this.UNreg.test(this.username)) {
        ...
        this.pwd = ''
      } else if (this.againPwd !== this.pwd) {
        this.$message({
          message: '两次密码不同',
          type: 'error'
        })
